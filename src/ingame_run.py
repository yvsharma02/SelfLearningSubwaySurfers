...
class InGameRun:
    ...
    def get_command_elim_window(self, action):
        ...
        if action == constants.ACTION_UP:
            return 0.1, 1.1 + (random.random() - 0.5) * 2 * .25 # torch.normal(1, .1, size=(1,)).item()
        if action == constants.ACTION_DOWN:
            return 0.1, 0.65 + (random.random() - 0.5) * 2 * .125 #0.1, torch.normal(0.7, .125, size=(1,)).item()
        if action == constants.ACTION_LEFT:
            return 0.1, 0.65 + (random.random() - 0.5) * 2 * .125 #0.1, torch.normal(.6, .1, size=(1,)).item()
